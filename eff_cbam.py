...
def train_cbam(config_path="train_config.yaml"):
    ...
    target_labels=None
    train_dataset = IP102Dataset2(config['train_annotation'], config['image_dir'], transform=train_transform)
    val_dataset = IP102Dataset2(config['val_annotation'], config['image_dir'], transform=val_transform)
    ...
    for epoch in range(config['epochs']):
        ...
        if acc > best_acc:
            best_acc = acc
            early_stop_counter = 0
            torch.save(model.state_dict(), os.path.join(config['log_dir'], 'aug_m_cbam_best_model.pth'))
        else:
